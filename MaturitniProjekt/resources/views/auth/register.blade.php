    ...
    <div class="container">
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <input
                ...
                name="name"
                value="{{ old('name') }}"
                placeholder="{{ $errors->has('name') ? $errors->first('name') : 'Zadejte své jméno' }}"
            ...
            <input
                ...
                placeholder="{{ $errors->has('password_confirmation') ? $errors->first('password_confirmation') : 'Potvrďte heslo' }}"
                class="{{ $errors->has('password_confirmation') ? 'error' : '' }}"
                required>

            <button type="submit" class="btn">Registrovat</button>
        </form>
        <a href="{{ route('login') }}" class="link">Již máte účet? Přihlaste se</a>
    </div>
</body>
